...
def affine_forward(x, w, b):
    ...
    out = x_new.dot(w) + b.reshape(1,len(b))
...
def relu_backward(dout, cache):
    ...
    dx = dout*(x>0)
    # (x>0) because - consider derivatives that are positive(ones that were 
    # passed as the output of relu)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    dgamma = (z*dout).sum(axis = (0,2,3), keepdims = True)
    dz = (gamma*dout).reshape(size).T
    z = z.reshape(size).T
    M = size[1]
    
    dx = dz/std - dz.sum(0)/(M*std) - np.sum(dz*z/std, 0)*z/M 
